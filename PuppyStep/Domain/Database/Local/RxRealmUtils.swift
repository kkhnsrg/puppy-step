...
extension Reactive where Base == Realm {
    ...
    public func removeData(_ data: Object) -> Single<Void> {
        return Single<Void>.create { single in
            ...
            do {
                ...
                if let removable = base.objects(DogDBO.self).filter(predicate).first {
                    try base.write {
                        base.delete(removable)
                        single(.success(()))
                    }
                }
            } catch let e {
    ...
    public func getData() -> Single<Object?> {
        return Single<Object?>.create { single in
            do {
                try base.write {
                    let data = base.objects(DogDBO.self).first
                    single(.success(data))
                }
            } catch let e {
                single(.error(e))
    ...
    public func addDataIfEmpty(_ data: Object) -> Single<Bool> {
        return Single.create { single in
            do {
                try
                    base.write {
                        let dataModel = base.objects(DogDBO.self).first
